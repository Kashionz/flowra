...
import {
  DEV_MODE_USER,
  isFlowraDevMode,
  readDevModeCloudBackup,
  writeDevModeCloudBackup,
} from "./flowraDevMode.js";
...
export function isCloudSyncAvailable() {
  return isFlowraDevMode() || isSupabaseConfigured();
}

export function getSupabaseConfigHint() {
  if (isFlowraDevMode()) {
    return "開發者模式已啟用，雲端功能改用本機 mock。";
  }
  return "雲端備份目前尚未開啟。";
}
...
export function createFlowraSupabaseClient() {
  if (isFlowraDevMode()) return null;
  const { url, key } = readRuntimeConfig();
...
export async function getCurrentSupabaseUser() {
  if (isFlowraDevMode()) {
    return { user: DEV_MODE_USER, error: null };
  }
  const supabase = createFlowraSupabaseClient();
  if (!supabase) {
...
export async function getLatestCloudBackup() {
  if (isFlowraDevMode()) {
    ...
    if (!storage) {
      return { data: null, error: new Error("開發者模式雲端備份需要瀏覽器 localStorage。") };
    }

    const data = readDevModeCloudBackup(storage);
...
export async function upsertCloudBackup({ payload }) {
  if (isFlowraDevMode()) {
    ...
    return { data: writeDevModeCloudBackup(storage, payload), error: null };
  }
  const supabase = createFlowraSupabaseClient();
  if (!supabase) {
    return { data: null, error: new Error(getSupabaseConfigHint()) };
