...
const ZH_ERROR_MESSAGES = [
  ...
  { pattern: /email link is invalid or has expired/i, message: "驗證連結已失效，請重新寄送驗證信。" },
  ...
  { pattern: /password should be at least (\d+)/i, translate: (match) => `密碼長度至少需 ${match[1]} 個字元。` },
  ...
  { pattern: /(failed to fetch|network ?request ?failed|networkerror)/i, message: "網路連線異常，請確認網路後再試。" },
  { pattern: /timeout|timed out/i, message: "連線逾時，請稍後再試。" },
  { pattern: /(permission denied|not authorized|unauthorized)/i, message: "權限不足，請重新登入後再試。" },
  { pattern: /row[- ]level security/i, message: "權限不足，請重新登入後再試。" },
  { pattern: /duplicate key|already exists/i, message: "資料已存在，請改用更新或還原。" },
  { pattern: /payload too large|request entity too large/i, message: "資料過大，無法上傳，請先移除部分內容。" },
  { pattern: /(internal server error|server error|service unavailable|bad gateway|gateway timeout)/i, message: "雲端服務暫時無法回應，請稍後再試。" },
];
