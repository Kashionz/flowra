...
export const PENDING_CLOUD_SYNC_STORAGE_KEY = "flowra.cashflow.pending-cloud-sync";
...
export function writePendingCloudSync(storage, payload, updatedAt) {
  storage?.setItem?.(PENDING_CLOUD_SYNC_STORAGE_KEY, JSON.stringify({ payload, updatedAt }));
}
...
export function resolveHydrationSource({
  localDraft,
  cloudPayload,
  cloudUpdatedAt = null,
  ...
  // Both cloud and local exist. Cloud is authoritative unless the local draft
  // carried unsynced changes from a *prior* session (pending record present at
  // mount) whose timestamp is strictly newer than the cloud's last update.
  // Pending records written during the current loading window are ignored —
  // those are edits made on top of the default scenario before cloud was
  // consulted, and must not clobber the cloud copy.
  const localHasNewerUnsyncedChanges =
    pendingExistedAtMount &&
    pendingUpdatedAtAtMount &&
    cloudUpdatedAt &&
    Date.parse(pendingUpdatedAtAtMount) > Date.parse(cloudUpdatedAt);

  if (localHasNewerUnsyncedChanges) {
    return { source: "draft", payload: localDraft };
  }
  return { source: "cloud", payload: cloudPayload };
}
