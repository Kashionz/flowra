# Flowra

Flowra 是一個前端導向的個人現金流試算工具，用來模擬未來幾個月的收入、固定支出、一次性收支與分期付款，並用圖表快速看出月底現金變化。

目前專案已包含：

- 現金流試算主頁
- 月底現金趨勢圖
- 每月收入與支出圖
- 支出組成堆疊面積圖
- 一次性收支與分期清單管理
- JSON / PNG / PDF / XLSX / 列印匯出
- Supabase 雲端備份與還原最近備份

## 技術棧

- React 19
- Vite 7
- Recharts
- shadcn-style chart wrapper
- Supabase JS
- dnd-kit
- html-to-image
- jsPDF
- SheetJS (`xlsx`)

## 本機開發

先安裝依賴：

```bash
pnpm install
```

啟動開發伺服器：

```bash
pnpm dev
```

預設網址：

```text
http://127.0.0.1:5173/
```

## 建置檢查

```bash
npm run build:check
```

這個指令會用 `esbuild` 檢查主檔是否可正常 bundle。

## 測試

```bash
pnpm test          # node --test，跑 lib/*.test.js（純函式單元測試，~111 tests）
pnpm test:e2e      # Playwright，跑 e2e/*.spec.js（desktop + iPhone SE viewport）
pnpm test:e2e:ui   # Playwright UI mode，互動式除錯
```

第一次跑 e2e 前需要先安裝瀏覽器：

```bash
pnpm exec playwright install chromium
```

如果你是在 Codex Desktop 內執行，而且環境變數是 `CODEX_SANDBOX=seatbelt`，`pnpm test:e2e` 會先擋下來並提示原因。這是因為該沙盒會阻止 Playwright 啟動 Chromium / Chrome，瀏覽器會在啟動階段就因為 macOS 的 `bootstrap_check_in ... Permission denied` 而中止。

這種情況請改用以下其中一種方式：

- 允許該命令在 Codex 內以非沙盒模式執行
- 在本機終端機直接執行 `pnpm test:e2e`
- 只有在你刻意要重現這個沙盒錯誤時，才加上 `PLAYWRIGHT_ALLOW_SANDBOXED_BROWSER=1`

CI 會自動跑兩種測試。

## 專案結構

```text
.
├─ components/
│  └─ ui/
│     └─ chart.jsx              # recharts wrapper + CHART_THEME_VARS
├─ hooks/
│  ├─ useCloudSync.js           # Supabase auth + 同步狀態機
│  └─ useScenarioHistory.js     # Undo / redo（包 lib/historyStack）
├─ lib/
│  ├─ clampViewport.js          # tooltip / dropdown 視窗邊界 clamp
│  ├─ dataManagementOptions.js  # 匯入 / 匯出 / 同步選單常數
│  ├─ expenseCategories.js      # 支出分類 metadata
│  ├─ finance.js                # buildProjection、monthlyPayment、純函式
│  ├─ flowraSupabase.js         # Supabase client + auth / sync helper
│  ├─ historyStack.js           # 上限 50 步的 past / future 堆疊
│  ├─ hydrationNotice.js        # 雲端 vs 本機 banner 決策
│  ├─ importDiff.js             # JSON 匯入預覽差異
│  ├─ initialBoot.js            # readInitialBoot + isScenarioEmpty
│  ├─ itemIds.js                # 一次性 / 分期項目 id 生成
│  ├─ jpyExchangeRate.js        # JPY → TWD 匯率快取與 fetch
│  ├─ numberField.js            # 零值輸入正規化、數字步進
│  ├─ scenarioPersistence.js    # localStorage 草稿 + 雲端 hydration 決策
│  ├─ templates/index.js        # 預設空白情境
│  └─ utils.js                  # cn() class merge
├─ scripts/                     # 部署 / 後端檢查腳本
├─ styles/
│  ├─ flowra.css                # 由 pnpm build:css 產出，勿手改
│  └─ flowra.tailwind.css       # Tailwind 來源
├─ supabase/
│  └─ migrations/
├─ index.html
├─ main.jsx
└─ personal_finance_cashflow_simulator.jsx
```

每個 `lib/*.js` 都有對應的 `lib/*.test.js`（`node --test` 執行，目前共 111 個 tests）；hooks 透過 lib/ 的純函式來測。

## 環境變數

可參考 `.env.example`：

```env
FLOWRA_SUPABASE_URL=https://your-project-ref.supabase.co
FLOWRA_SUPABASE_PUBLISHABLE_KEY=your-publishable-key
VITE_SUPABASE_URL=https://your-project-ref.supabase.co
VITE_SUPABASE_PUBLISHABLE_KEY=your-publishable-key
```

本機開發通常使用：

- `.env.local`
- `VITE_SUPABASE_URL`
- `VITE_SUPABASE_PUBLISHABLE_KEY`

另外可用以下環境變數啟用開發者模式：

```env
VITE_FLOWRA_DEV_MODE=1
```

## 開發者模式

用途：

- 在本機開發時，不需要先登入 Google 帳號，就能測試雲端備份、還原備份、AI 輔助分析
- 讓 UI / 狀態切換 / 比較流程可以快速驗證，不必每次都接上真實 Supabase auth 與 Edge Function

啟用方式：

```bash
VITE_FLOWRA_DEV_MODE=1 pnpm dev
```

或寫進 `.env.local`：

```env
VITE_FLOWRA_DEV_MODE=1
```

開啟後的行為：

- 帳號區塊會顯示為已登入，並標示「開發者模式」
- 雲端備份與還原最近備份會改用瀏覽器 `localStorage` mock 資料，不會寫入真實 Supabase
- AI 輔助分析會回傳本機 mock 結果，不會呼叫 `ai-scenario` Edge Function，也不需要 `ANTHROPIC_API_KEY`
- 重新整理頁面後，若先前已有開發者模式備份，仍會走 mock 的雲端 hydration 流程

限制：

- 這不是正式登入，也不代表 RLS、auth session、Edge Function 權限、配額等真實雲端行為已驗證
- AI 回覆是固定 mock 情境，適合驗證前端流程，不適合驗證模型品質或提示詞效果
- 開發者模式的雲端備份資料只存在目前瀏覽器的本機儲存，不會跨裝置同步
- 若你要驗證正式的登入、Supabase 備份、Edge Function、`ANTHROPIC_API_KEY`、每日額度等行為，請關閉 `VITE_FLOWRA_DEV_MODE`，改用真實環境測試

## Supabase 設定

若要啟用雲端備份，除了環境變數以外，還要先套用 migration：

```text
supabase/migrations/20260505162604_flowra_cloud.sql
supabase/migrations/20260506120000_flowra_share_view_count_rpc.sql
supabase/migrations/20260506123000_flowra_single_backup.sql
```

目前雲端能力包含：

- 同步最新備份
- 還原最近備份

若你只是要本機開發與驗 UI 流程，可改用上面的「開發者模式」，不需要真實登入。

## AI 輔助分析

Flowra 的「AI 輔助分析」功能：用自然語言說明你想模擬的變化（買車、換工作、退休），AI 會回傳一份結構化情境修改提議，前端套用後與當前情境並排比較。

需求：

- 已啟用 Supabase（同 cloud sync）
- 已登入
- 在 Supabase 設定 secret：`ANTHROPIC_API_KEY`
- 部署 Edge Function：`supabase functions deploy ai-scenario`
- 套用 migration：`supabase/migrations/20260508120000_flowra_ai_usage_log.sql`

每位使用者每日上限 20 次呼叫。AI 不會修改你的起始資產（startingTwd / jpyCash）；它只能新增/修改/移除一次性項目、分期，與部分基本欄位（薪水、補助、房租、生活費、學貸、月份範圍）。

若已啟用 `VITE_FLOWRA_DEV_MODE=1`，這一節的正式需求會被 mock 取代，方便開發時直接測試抽屜、提議卡、比較與套用流程。

## 目前產品行為

這個版本是依照目前需求收斂後的版本，幾點需要先知道：

- 不保留本機草稿內容，只保留時間資訊
- 已移除 Sankey 金流圖
- 已移除月度熱力圖
- 已移除草稿與範本 UI
- 已移除隱私模式
- 已移除「還原預設」操作

## 匯出能力

目前支援：

- 整頁 PNG
- 單張圖 PNG
- PDF 報表
- Excel 明細
- JSON 匯出
- 列印

## 文件

- 原始需求規格：[`flowra-spec.md`](./flowra-spec.md)
- Supabase migration runbook：[`docs/supabase-migration-runbook.md`](./docs/supabase-migration-runbook.md)
- Supabase 遠端驗證清單：[`docs/supabase-remote-verification-checklist.md`](./docs/supabase-remote-verification-checklist.md)
